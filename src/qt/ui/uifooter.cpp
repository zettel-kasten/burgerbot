...
UIFooter::UIFooter(QWidget *parent) : QWidget(parent)
{
    ...
    for(int i = 0; i<label_text.count();i=i+2){
        ...
        label[i+1]->setStyleSheet(".QLabel{color:#EEBF1F; font: 11px \"Open Sans\";}");
...
void UIFooter::updateDataLabels()
{
    ...
    if(itr != NULL){
        ...
        label[3]->setText(QString::number( totalMoney)+" BURGER");
...
void UIFooter::paintEvent(QPaintEvent *e)
{
    ...
    if(show_preloader)
    {
        ...
        painter.fillRect(200,55,(this->width()-200-120)*(qreal)count/(qreal)nTotalBlocks,10,QColor(0xFFDF2F));
