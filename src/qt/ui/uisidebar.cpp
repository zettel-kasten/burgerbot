...
void UIIconLabel::setActive(bool active_)
{
    ...
    label->setStyleSheet(".QLabel{color:#FFFFFF; font: bold 16px \"Open Sans\";}");
    if(active){
        label->setStyleSheet(".QLabel{color:#FFDF2F; font: bold 16px \"Open Sans\";}");
    }
    update();
}
...
void UIIconLabel::paintEvent(QPaintEvent *e)
{
    ...
    if(pixmap_path != ""){
        ...
        if(active || hover){
            pix = PixmapEffect::tint(pix,0xFFDF2F);
        } else{
    ...
    if(active){
        painter.fillRect(0,8,20,2,QColor(0xFFDF2F));
    }
...
void UISideBar::paintEvent(QPaintEvent *e)
{
    ...
    painter.begin(this);
    painter.fillRect(0,0,200,this->height(),QColor(0x9c6f41));

    //painter.fillRect(0,0,200,2,QColor(0x3b3b3b));//3b3b3b));

    QPixmap pix_logo(":/icons/bitcoin");

    painter.drawPixmap(30,this->height()-88-30,pix_logo);
